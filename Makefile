# Project Setup
PROJECT_NAME := platform-ref-aws-castai
...
uptest: $(UPTEST) $(KUBECTL) $(KUTTL)
	...
	@KUBECTL=$(KUBECTL) KUTTL=$(KUTTL) CROSSPLANE_NAMESPACE=$(CROSSPLANE_NAMESPACE) $(UPTEST) e2e examples/cluster.yaml --data-source="${UPTEST_DATASOURCE_PATH}" --setup-script=test/setup.sh --default-timeout=4800 || $(FAIL)
...
render:
	crossplane beta render examples/cluster.yaml apis/composition.yaml examples/functions.yaml -r
yamllint:
